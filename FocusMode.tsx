    ...
    <div className="fixed inset-0 z-[200] bg-background flex flex-col items-center justify-center p-6 md:p-12 overflow-hidden">
      ...
      <div className="w-full max-w-3xl space-y-12 text-center relative z-10">
        <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-4">
          ...
          <h2 className="text-5xl md:text-8xl font-black tracking-tighter text-text_primary drop-shadow-2xl">{task.title}</h2>
        </motion.div>

        <div className="relative size-80 md:size-[450px] mx-auto flex items-center justify-center">
          <svg className="absolute inset-0 size-full -rotate-90">
            <circle cx="50%" cy="50%" r="46%" fill="none" stroke="var(--color-border)" strokeWidth="12" />
            <motion.circle
              cx="50%" cy="50%" r="46%"
              fill="none" stroke="var(--color-primary)" strokeWidth="12"
              strokeDasharray="100" strokeDashoffset={100 - progress}
              pathLength="100" className="transition-all duration-1000" strokeLinecap="round"
            />
          </svg>
          <div className="flex flex-col items-center">
            ...
            <div className="text-xs md:text-sm font-black uppercase tracking-[0.5em] text-text_secondary mt-4">Remaining</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
          ...
          <div className="glass-card p-8 bg-surface border-border flex flex-col items-center justify-center gap-2">
            <div className="text-xs font-black uppercase tracking-widest text-text_secondary">Focus Score</div>
            <div className="text-4xl font-black text-secondary">
              {distractions === 0 ? '100' : Math.max(60, 100 - distractions * 8)}%
            </div>
            <div className="text-xs font-bold text-text_secondary uppercase tracking-widest">
              {distractions === 0 ? 'Optimal Flow' : distractions < 3 ? 'Stay Sharp' : 'Refocus'}
            </div>
          </div>
        </div>
