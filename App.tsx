...
import { auth, googleProvider, db, logEvent, OperationType, handleFirestoreError } from './firebase';
...
export default function App() {
  ...
  // Fix: Populate timelineMatrix from missions that have startTime assigned
  useEffect(() => {
    const scheduled = missions
      .filter(m => m.startTime && m.status !== 'completed')
      .sort((a, b) => (a.startTime || '').localeCompare(b.startTime || ''));
    setTimelineMatrix(scheduled);
  }, [missions]);
  ...
  useEffect(() => {
    if (firebaseUser && missions.length > 0) {
      checkStreaks(firebaseUser.uid, missions).then(streaks => {
        if (motivationState) {
          setMotivationState({ ...motivationState, current_streak: streaks.current, focus_streak: streaks.focus });
        }
      });
      ...
      <div className="space-y-8 md:space-y-10">
        <form onSubmit={saveMission} className="stitch-card p-4 md:p-10 space-y-6 md:space-y-8 border-border relative overflow-hidden">
          ...
          <div className="space-y-6 md:space-y-8 relative z-10">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-text_secondary uppercase tracking-widest ml-1">
                {theme.id === 'elite' ? 'Mission Category' : 'Category'}
              </label>
              <div className="flex flex-wrap gap-2">
                {['work', 'health', 'personal', 'learning', 'general'].map(cat => (
                  <button
                    key={cat}
                    type="button"
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${
                      category === cat
                        ...
                        : 'bg-surface text-text_secondary border-border hover:border-primary/40 hover:text-text_primary'
                    }`}
                  >
                    {cat}
                  </button>
          ...
          {missions
            ...
            .map((mission: Mission) => (
              <MissionCard
                key={mission.id}
                mission={mission}
                onComplete={(id, streak) => handleAction('COMPLETE_TASK', { id, streak })}
                onDelete={(id) => handleAction('DELETE_TASK', { id })}
                onStartFocus={(task) => handleAction('START_FOCUS', { task })}
                onEdit={startEdit}
                themeId={theme.id}
                animationsType={theme.animations.type}
              />
            ))}
          {missions.filter(m => {
            if (taskFilter === 'all') return true;
            ...
            <div className="flex items-center justify-between p-4 bg-surface rounded-2xl border border-border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] font-medium text-text_secondary">{userProfile?.subscription_plan === 'premium' ? 'Elite Access Active' : 'Standard Protocol'}</div>
                </div>
              </div>
  ...
  const fetchNextAction = async () => {
    ...
    const sorted = [...pendingTasks].sort((a, b) => {
      ...
      const scoreA = ((a.urgency_score || 5) * 0.4) + ((a.impact_level || 5) * 0.4) + ((a.urgency_score || 5) * 0.2);
      const scoreB = ((b.urgency_score || 5) * 0.4) + ((b.impact_level || 5) * 0.4) + ((b.urgency_score || 5) * 0.2);
  ...
  return (
    <ErrorBoundary>
      ...
      <AnimatePresence>
        {showAiPanel && <AiPanel onClose={() => setShowAiPanel(false)} missions={missions} consistencySystem={consistencySystem} userProfile={userProfile} />}
      </AnimatePresence>
