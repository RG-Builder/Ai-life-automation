...
export const TimelineMatrix: React.FC = () => {
  ...
  return (
    ...
    >
      <motion.div variants={theme.motion.variants.item} className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 px-0">
        <div>
      ...
      <motion.div variants={theme.motion.variants.container} className="relative pl-10 sm:pl-14 space-y-8 sm:space-y-12">
        {/* Timeline Line */}
        <motion.div 
          initial={{ height: 0 }}
          animate={{ height: '100%' }}
        ...
        {timelineMatrix.length > 0 ? timelineMatrix.map((mission, idx) => (
          <motion.div key={mission.id} variants={theme.motion.variants.item} className="relative group">
            ...
            <motion.div 
              whileHover={theme.motion.hover}
              className="stitch-card stitch-card-hover p-6 sm:p-8 border-border relative overflow-hidden"
            >
                ...
                <div className="flex items-center gap-3 ml-auto sm:ml-0">
                  {mission.status !== 'completed' && (
                    <motion.button 
                      ...
                      className="px-4 py-2 bg-primary/10 text-primary rounded-lg text-[10px] font-black uppercase tracking-widest border border-primary/20 hover:bg-primary hover:text-black transition-all"
                    >
                      {theme.id === 'elite' ? 'Engage' : 'Focus'}
                    </motion.button>
                  )}
                  <motion.button 
                    whileHover={theme.motion.hover}
          ...
          <motion.div 
            ...
            className={`stitch-card p-12 sm:p-24 text-center space-y-6 border-dashed ${theme.id === 'elite' ? 'border-border' : 'border-primary/20'}`}
          >
