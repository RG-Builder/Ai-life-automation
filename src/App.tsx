...
export default function App() {
  ...
  const generateDayPlan = async () => {
    if (missions.length === 0) {
      console.warn("No missions to plan.");
      return;
    }
    ...
    const behavioralContext = {
      userProfile,
      lifeState,
      consistencySystem: consistencySystem.map(h => ({ title: h.title, streak: h.streak, current_count: h.current_count, goal_count: h.goal_count })),
      selfAwareness,
      motivationState,
    ...
    User Behavioral Context: ${JSON.stringify(behavioralContext)}
    
    Guidelines for Personalization:
    1. Consider the user's current energy and focus levels (${lifeState.status}, Focus: ${lifeState.focusLevel}%). Schedule high-impact tasks during peak states.
    2. Respect existing streaks and habits. If the user is in 'Recovery Mode', prioritize smaller, achievable wins.
    3. Start the schedule from the current time (${behavioralContext.currentTime}) or shortly after.
    ...
    try {
      console.log("Generating personalized day plan with AI...");
        ...
        const updates = plan.map(async (item) => {
          if (item.id && item.startTime && item.endTime) {
            try {
              ...
              // Check if document exists before updating to avoid errors
              ...
              if (docSnap.exists()) {
                ...
                console.warn(`Task ${item.id} not found, skipping schedule update.`);
              }
            } catch (err) {
              console.error(`Failed to update task ${item.id}:`, err);
            }
          }
        });
  ...
  const generateAiInsights = async () => {
    const prompt = `Analyze user performance data and provide 3 actionable, high-impact insights tailored to their current behavioral state.
    
    Current State: ${JSON.stringify({ missions, consistencySystem, lifeState, motivationState, selfAwareness })}
    
    Focus on:
    1. Identifying patterns in task completion vs. energy levels.
    2. Suggesting optimizations for habit consistency.
    3. Providing psychological nudges based on their current streak or recovery status.
  ...
  const handleAction = async (type: string, payload: any = {}) => {
    ...
    try {
      switch (type) {
        ...
        case 'START_FOCUS': {
          console.log("Starting focus mode for task:", cleanPayload.task);
  ...
  const generateSchedule = async () => {
    if (missions.length === 0) {
      alert("Please add some missions first!");
      return;
    }
    setLoading(true);
